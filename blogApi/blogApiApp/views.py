...
def deletePost(request):
    ...
    try:
        ...
        post.delete()
        return Response({'success':'Post successfully deleted'},status=201)
...
def  GetPost(request):
    ...
    try:
        ...
        return Response({'Error':'The Post does not exist'},status=40)
    
@api_view(['PUT']) 
def  UpdatePost(request):
    post_id = request.data.get('post_id')
    get_new_title = request.data.get('post_id')
    get_new_content = request.data.get('new content')
    try:
        post = Post.objects.get(id=post_id)
        if get_new_title:
        ...
        post.save()    
        return Response({'success':'post successfully updated'},status=200)
